...
- Equation decks (`configs/equations/{statistics,physics,mathematics}.yaml`): the `item` is the canonical LaTeX, so retuning the corruption engine, retiring a type, or an equation moving between the 2-error/1-error decks never strands FSRS history (unlike the monarch end-year change). Corruptions are generated, equations are curated. One config → two decks (2-error / 1-error, split by `classify`), disambiguated by `poem_title`. `normalise.py` unlocked real notation (`P(A\mid B)`, `\operatorname{Var}`), so the earlier stand-in workaround is gone; vector calculus (`\nabla \times`) still fails closed. See [docs/design/equations-pipeline.md](docs/design/equations-pipeline.md)
