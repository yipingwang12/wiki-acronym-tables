...
### Quiz event logger
All quiz sessions are logged to `logs/quiz.db` (SQLite, gitignored). Two tables:
...
`item_key` is stable across sessions and config changes, enabling future SRS scheduling per item.
...
### Planned: spaced repetition system (SRS)
Logging is the prerequisite. SRS will use `item_key` to schedule items by performance history, and `format_version` to exclude stale sessions from scheduling calculations.
