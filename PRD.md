...
**Hand-authored `source: manual` artifacts are preserved through the clear.** Some decks the quiz consumes are hand-authored (e.g. `memory-quiz-app`'s CC-CEDICT Chinese vocab deck for matching mode) and have **no config here to rebuild from** — a full run would silently delete them. `export_decks` skips any artifact whose JSON carries `"source": "manual"` (`_is_manual`; unreadable/malformed → treated as generated, so a corrupt generated deck is still rebuilt). The orchestrator's Dagster `decks` sync applies the same guard when clearing the quiz repo's dir.
