...
from deck_generator.gutenberg import fetch_text
...
def test_fetches_and_returns_text(tmp_path):
    with patch("deck_generator.gutenberg.requests.get", return_value=_mock_response(_FAKE_TEXT)) as mock_get:
...
def test_caches_to_disk(tmp_path):
    with patch("deck_generator.gutenberg.requests.get", return_value=_mock_response(_FAKE_TEXT)):
...
def test_cache_hit_skips_network(tmp_path):
    ...
    with patch("deck_generator.gutenberg.requests.get") as mock_get:
        result = fetch_text(1041, cache_dir=tmp_path)
...
def test_different_ids_cached_separately(tmp_path):
    with patch("deck_generator.gutenberg.requests.get", return_value=_mock_response("book A")):
        fetch_text(100, cache_dir=tmp_path)
    with patch("deck_generator.gutenberg.requests.get", return_value=_mock_response("book B")):
...
def test_http_error_propagates(tmp_path):
    ...
    with patch("deck_generator.gutenberg.requests.get", return_value=resp):
